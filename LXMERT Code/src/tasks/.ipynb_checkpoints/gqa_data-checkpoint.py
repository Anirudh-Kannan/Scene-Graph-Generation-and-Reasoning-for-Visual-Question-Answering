...
class GQADataset:
    ...
    def __init__(self, splits: str):
        ...
        for split in self.splits:
            self.data.extend(json.load(open(f"{data_dir}/{split}.json")))
        ...
        # Answers
        # Answers
        self.ans2label = json.load(open(f"{data_dir}/trainval_ans2label.json"))
        self.label2ans = json.load(open(f"{data_dir}/trainval_label2ans.json"))
        assert len(self.ans2label) == len(self.label2ans)
        for ans, label in self.ans2label.items():
            assert self.label2ans[label] == ans
            
#         ans2label = json.load(open(f"{data_dir}/trainval_ans2label.json"))
#         label2ans = json.load(open(f"{data_dir}/trainval_label2ans.json"))
        
#         self.ans2label = {}
#         self.label2ans = {}
#         for k, v in ans2label.items():
#             self.ans2label[k] = int(v)
            
#         for k, v in enumerate(label2ans):
#             self.label2ans[int(k)] = v
